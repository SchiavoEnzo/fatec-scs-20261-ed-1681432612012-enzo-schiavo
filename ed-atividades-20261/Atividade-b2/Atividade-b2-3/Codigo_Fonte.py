...
class ArvoreBST:
        ...
        if self.raiz is None:
            ...
            while (pointer):
                if pointer.valor >= elem:
                    ...
                    if pointer.dir is None:
                        pointer.dir = node
                        return
                    pointer = pointer.dir
                else:
                    
                    if pointer.esq is None:
                        pointer.esq = node
                        return
                    pointer = pointer.esq
    ...
    def analisar_arvore(self, valor_busca):
        ...
        def l(no):
            ...
            if no.valor == valor_busca:
                print(f"Altura do nó: {self.calcular_altura(no)}")
                print(f"Id do nó: {id(no)}")
                return
            l(no.esq)
            l(no.dir)
        l(self.raiz)
    ...
    def calcular_profundidade(self, valor):
        def lookFor(no, p):
            ...
            return r
        print(f"Profundidade do nó: {lookFor(self.raiz, 0)}")
